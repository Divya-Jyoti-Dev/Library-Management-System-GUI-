...
public class User implements Serializable {
    ...
    public Map<String, Integer> overdueReminderCount = new HashMap<>();
    public Map<String, LocalDateTime> overdueReminderLastSent = new HashMap<>();
    ...
    public void returnBook(String isbn, String title) {
        ...
        if (borrowingHistory.size() > 50)
            borrowingHistory.remove(0);
        if (overdueReminderCount == null)
            overdueReminderCount = new HashMap<>();
        if (overdueReminderLastSent == null)
            overdueReminderLastSent = new HashMap<>();
        overdueReminderCount.remove(isbn);
        overdueReminderLastSent.remove(isbn);
    }

    public double calculateFine() {
        LocalDate today = LocalDate.now();
        double gross = issuedBooks.values().stream()
                .filter(today::isAfter)
                .mapToLong(d -> today.toEpochDay() - d.toEpochDay())
                .sum() * LibraryConfig.FINE_PER_DAY;
        return Math.max(0, gross - finePaid);
    }
