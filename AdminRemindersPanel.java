...
public class AdminRemindersPanel extends JPanel {
    ...
    public AdminRemindersPanel(HashMap<String, User> userDb, Library library, User adminUser) {
        ...
        JLabel note = UITheme.label(
                "ℹ  Overdue books: 3 notices sent 1 hour apart, then every 3 days until returned. Near-due reminders sent within "
                        + REMIND_DAYS + " days.",
    ...
    public void refresh() {
        ...
        for (User u : userDb.values()) {
            if (u.isAdmin || u.issuedBooks == null || u.issuedBooks.isEmpty())
                continue;
            if (u.overdueReminderCount == null)
                u.overdueReminderCount = new HashMap<>();
            if (u.overdueReminderLastSent == null)
                u.overdueReminderLastSent = new HashMap<>();
    ...
    private void sendAutoReminders() {
                ...
                if (days >= 0) {
                    if (days <= REMIND_DAYS) {
                        u.inbox.add(new Message("SYSTEM", u.email,
                                ...
                                ReminderHelper.body(u.email, title, due, days, sign),
                                Message.Type.AUTO_DUE_REMINDER));
                        sent++;
                    }
                } else {
                    int count = u.overdueReminderCount.getOrDefault(isbn, 0);
                    java.time.LocalDateTime lastSent = u.overdueReminderLastSent.getOrDefault(isbn, null);
                    ...
                    if (count == 0) {
                        ...
                        shouldSend = lastSent != null &&
                                java.time.Duration.between(lastSent, now).toHours() >= 1;
                    } else {
                        shouldSend = lastSent != null &&
                                java.time.Duration.between(lastSent, now).toDays() >= 3;
                    }

                    if (shouldSend) {
                        int reminderNum = count + 1;
                        u.inbox.add(new Message("SYSTEM", u.email,
                                ReminderHelper.overdueSubject(title, reminderNum),
                                ReminderHelper.overdueBody(u.email, title, due, days, reminderNum, sign),
                                Message.Type.OVERDUE_REMINDER));
                        u.overdueReminderCount.put(isbn, reminderNum);
                        u.overdueReminderLastSent.put(isbn, now);
                        sent++;
                    }
                }
            }
        ...
        statusLabel.setText(sent > 0 ? "✓ " + sent + " reminder(s) sent." : "No reminders due right now.");
    }
