...
public class MainFrame extends JFrame {
    ...
    private JTable buildBookTable() {
        ...
        // Double-click → book detail popup
    ...
    private void showBookDetail(Book b) {
        ...
        addDetailRow(info, "Publisher:",
                (b.getPublisher() != null && !b.getPublisher().isEmpty()) ? b.getPublisher() : "N/A");
    ...
    private void doBorrow() {
        ...
        if (library.issueBook(b.getIsbn())) {
            ...
            b.clearReservationNotified(currentUser.email);
        ...
        if (nextEmail != null) {
            ...
            if (waiting != null) {
                ...
                waiting.inbox.add(new Message("SYSTEM", nextEmail, "Book Available: \"" + b.getTitle() + "\"",
                        "A copy of \"" + b.getTitle()
                                + "\" is now available. You are #1 in queue.\n\nYou have 1 day to borrow it before your spot is passed to the next person in queue.",
                        Message.Type.AUTO_DUE_REMINDER));
                b.setReservationNotified(nextEmail);
            }
        }
        saveAll.run();
        refreshAll();
    ...
    private void expireStaleReservations() {
        for (Book b : library.getAllBooks()) {
            ...
            java.time.LocalDateTime notified = b.getReservationNotifiedAt(first);
            if (notified == null)
                continue;
            long hoursElapsed = java.time.Duration.between(notified, java.time.LocalDateTime.now()).toHours();
            if (hoursElapsed >= 24) {
                queue.remove(0);
                b.clearReservationNotified(first);
                User expired = userDb.get(first);
                if (expired != null) {
                    if (expired.inbox == null)
                        expired.inbox = new ArrayList<>();
                    expired.inbox.add(new Message("SYSTEM", first,
                            "Reservation Expired: \"" + b.getTitle() + "\"",
                            "Your reserved spot for \"" + b.getTitle()
                                    + "\" has expired because you did not borrow it within 1 day.\nYour position has been passed to the next member in queue.",
                            Message.Type.AUTO_DUE_REMINDER));
                }
                if (!queue.isEmpty()) {
                    String nextEmail = queue.get(0);
                    User next = userDb.get(nextEmail);
                    if (next != null) {
                        if (next.inbox == null)
                            next.inbox = new ArrayList<>();
                        next.inbox.add(new Message("SYSTEM", nextEmail,
                                "Book Available: \"" + b.getTitle() + "\"",
                                "A copy of \"" + b.getTitle()
                                        + "\" is now available. You are now #1 in queue.\n\nYou have 1 day to borrow it before your spot is passed to the next person.",
                                Message.Type.AUTO_DUE_REMINDER));
                        b.setReservationNotified(nextEmail);
                    }
                }
    ...
    private void showReservationPopups() {
        expireStaleReservations(); // expire any stale queue slots before showing popups
    ...
    private void showEditBookDialog() {
        ...
        if (r != null) {
            library.updateBook(dlg.getOldIsbn(), r.getTitle(), r.getAuthor(), r.getIsbn(), r.getTotalCopies(),
                    r.getCategory(), null, r.getPublisher());
            refreshAll();
        }
    }
